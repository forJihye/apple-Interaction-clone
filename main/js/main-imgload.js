...
(() => {
  ...
  // Scene 0에 쓰이는 scene0Images 이미지 배열을 번호순 정렬 후 sceneInfo[0].objs.videoImages 배열에 저장
  function setImagesOfScene0() {
		sortImages(scene0Images);
		for (let i = 0; i < scene0Images.length; i++) {				
			sceneInfo[0].elms.videoImages.push(scene0Images[i]);
		}
	}

  // Scene 2에 쓰이는 scene2Images 이미지 배열을 번호순 정렬 후 sceneInfo[2].objs.videoImages 배열에 저장
  function setImagesOfScene2() {
		sortImages(scene2Images);
		for (let i = 0; i < scene2Images.length; i++) {
			sceneInfo[2].elms.videoImages.push(scene2Images[i]);
		}
	}
  ...
  function initAfterLoadImages() {
		if (currentScene !== 2 && sceneInfo[0].elms.videoImages[0]) {
			sceneInfo[0].elms.context.drawImage(sceneInfo[0].elms.videoImages[0], 0, 0);
		}
  ...
  window.addEventListener('DOMContentLoaded', () => { 
    ...
    for (let i = 0; i < sceneInfo[3].elms.imagePath.length; i++) {
      ...
      imgElem.src = sceneInfo[3].elms.imagePath[i];
      sceneInfo[3].values.images.push(imgElem)
    }

    console.log('Scene 0번, Scene2번 loadImages 호출');
		if (currentScene !== 2) {
			// 0번, 첫번째 씬의 이미지를 로드
			loadImagesOfScene0();
		} else {
